...
const getInsightsCountFromDb = function getInsightsCountFromDb({ id, highWeight, mediumWeight, daysAgo }){
    const query = buildGetInsightsCountQuery(id, highWeight, mediumWeight, daysAgo);
    return snowflakeClientExecuteQuery(query);
}

const getArtistInsightsFromDb = function getArtistInsightsFromDb({ id, limit, weight, daysAgo }){
    const query = buildGetArtistInsightsQuery(id, limit * 10, weight, daysAgo);
    return snowflakeClientExecuteQuery(query);
}

const evaluateWeight = async function evaluateWeight ({id, daysAgo}) {
...
const createInsights = function createInsights({formattedInsights, isNewsFormat, limit}){
    const insights = [];
    let i = 0;
...
const getArtistInsights = async function getArtistInsights({ id, limit, weight, daysAgo, newsFormat }) { // assume that id, limit, weight, daysAgo, newsFormat r in correct type
    try{
